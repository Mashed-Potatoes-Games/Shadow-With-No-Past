...
class GridManagementEditor : Editor
{
    private WorldManagement grid;
    private const string labelsName = "CoordinateLabels";

    private void OnEnable()
    {
        grid = (WorldManagement)target;
        if(grid.ShowTileCoordinates)
        {
            AddCoordinateLabels();
        }
    }
    ...
    {
        Transform labelsTransform = grid.transform.Find(labelsName);
    ...
    private void SetupCanvas(GameObject Canvas)
    {
        ...
        Canvas.transform.SetParent(grid.transform);
